...
import { BookType, ChevronsUpDown, GraduationCap, Pencil, Printer, ScrollText, Trash2, UserRound } from "lucide-react"
import { Session, getServerSession } from "next-auth"

async function getData(session: Session, id: string) {
    const fetchApi =
        await fetch(`http://127.0.0.1:8000/data/api/v1/alumnos?num_control=${id}`,
            {
                method: "GET",
                headers: {
...
export default async function Page({ params }: { params: { id: string } }) {
    ...
    const [{ nombre, apellidop, apellidom, nombre_carrera, periodo_ingreso, periodo_egreso, sexo,
        CURP, estado_nacimiento, fecha_nacimiento, num_folio, fecha_registro_cert,
        observaciones_cert, num_titulo, clave_plan, fecha_acto, fecha_registro_tit,
        num_cedula, observaciones_tit
    }] = await getData(session, params.id)
    const sexoText = sexo === "m" || sexo === "M" ? "Masculino" : sexo === "f" || sexo === "F" ? "Femenino" : "Indefinido"
    return (
        <PageLayout>
            <h1 className="text-muted-foreground font-semibold mt-8 text-md">Número de control: {params.id}</h1>
            ...
            <section className="grid grid-cols-1 xl:grid-cols-2 gap-8 mb-10">
                <Collapsible defaultOpen>
                    <CollapsibleTrigger asChild>
                        <Button variant="ghost" size="sm" className="text-muted-foreground w-full justify-between text-xl">
                            ...
                            <CardContent>
                                <Table>
                                    <TableBody>
                                        ...
                                        { head: "Estado de nacimiento", value: estado_nacimiento },
                                ...
                                <CardContent>
                                    <Table>
                                        <TableBody>
                                            ...
                                            { head: "Fecha de registro", value: fecha_registro_cert },
                                ...
                                <CardContent>
                                    <Table>
                                        <TableBody>
                                            ...
                                            { head: "Fecha de registro", value: fecha_registro_tit },
