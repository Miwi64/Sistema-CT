...
export const DeleteButton = ({ id, mode = "button" }: DeleteButtonProps) => {
  ...
  const handleDelete = async (controlNumber: string) => {
    ...
    const { results: [student] } = await search.json()
    ...
    setOpen(false)
    if (!response.ok) {
      notification(`Error al intentar eliminar el registro (${response.status})`,
        "error", "Vuelva a intentarlo más tarde", isDesktop
      )
      return
    }
    if (pathname === "/table") {
      notification(`El registro ha sido eliminado correctamente`,
        "success", "Recargando la página", isDesktop
      )
      location.reload()
    }
    else {
  ...
  return (
    <ResponsiveDialog
      ...
      trigger={
        mode === "button" ?
          ...
          isDesktop ?
            ...
            >
              <Trash2 />
              <span className={"text-lg ml-4 md:inline"}>
                Eliminar
              </span>
            </Button>
      }
      ...
      <section className="flex items-center gap-3">
        <Button onClick={async () => { await handleDelete(id) }} variant="destructive">
          <Trash2 className="mr-2" />
          <span>Eliminar</span>
        </Button>
        <Button onClick={() => setOpen(!open)} variant="outline">Cancelar</Button>
      </section>
    </ResponsiveDialog>
  );
}
