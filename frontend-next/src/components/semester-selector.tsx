...
const SemesterSelector = (
    { from = 1960, to = new Date().getFullYear(), value, onValueChange, reverse }: SemesterSelectorProps) => {
    const [semesters, setSemesters] = useState<Semester[]>([]);
    useEffect(() => {
        const semestersArray: Semester[] = [];
        for (let year = from; year <= to; year++) {
            semestersArray.push({ value: selectSemesterValue(year, 1), text: `${year}-1` });
            semestersArray.push({ value: selectSemesterValue(year, 2), text: `${year}-2` });
        }
        if (reverse) {
